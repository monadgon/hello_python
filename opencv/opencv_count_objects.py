...
image = cv2.imread("C:\PjtPython\hello_python\opencv\coins.jpeg")
if image is None:
...
cv2.namedWindow("src1", flags=cv2.WINDOW_KEEPRATIO)
cv2.imshow("src1", image_blur)
cv2.waitKey(0)
...
cv2.namedWindow("src2", flags=cv2.WINDOW_KEEPRATIO)
cv2.imshow("src2", image_blur_gray)
cv2.waitKey(0)
...
cv2.namedWindow("last", flags=cv2.WINDOW_KEEPRATIO)
...
def display(img,count,cmap="gray"):
    cv2.namedWindow("last1", flags=cv2.WINDOW_KEEPRATIO)
    cv2.imshow("last1", img)
    cv2.waitKey(0)

    f_image = cv2.imread("C:\PjtPython\hello_python\opencv\coins.jpeg")
